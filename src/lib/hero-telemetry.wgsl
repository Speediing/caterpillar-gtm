...
fn agentComputerMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.08);
  let bezel = sdRoundedBox(q - vec2f(0.0, -0.03), vec2f(0.20, 0.13), 0.025);
  let screen = sdRoundedBox(q - vec2f(0.0, -0.03), vec2f(0.165, 0.095), 0.012);
  let stem = sdRoundedBox(q - vec2f(0.0, 0.125), vec2f(0.025, 0.055), 0.01);
  let base = sdRoundedBox(q - vec2f(0.0, 0.18), vec2f(0.105, 0.018), 0.009);
  var mark = opSmoothUnion(bezel, stem, 0.012);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let dComputer = agentComputerMark(computerP);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(yellow, paper, spark * 0.45 + line * 0.05);
